...
## Purpose
Defines hardened quality gates: per-module coverage floors for safety-critical modules enforced by `scripts/check_coverage.py`, fixture-based documentation consistency with no skips, ruff security rules with justified suppressions, and workflow structure plus action-pinning checks.
## Requirements
